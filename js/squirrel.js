...
function sqCardHTML(sq) {
  ...
  const borderColor = (sq.status === 'explorer' || sq.status === 'exploring') ? '#3b82f6' : sq.status === 'pet' ? '#ec4899' : '#a3a3a3';
  ...
  return `
    <div onclick="sqOpenModal('${sq.id}')" style="background:white;border-radius:20px;padding:16px;box-shadow:0 4px 20px rgba(0,0,0,0.07);border:2px solid transparent;border-left:4px solid ${borderColor};cursor:pointer;transition:all .2s">
      <div class="flex items-start gap-2">
        ...
        <div class="flex-1 min-w-0">
          ...
          <div class="text-xs text-gray-400 mt-0.5">${typeLabel[sq.status] || sq.status}</div>
        </div>
      </div>
      ${progressHTML}
    </div>`;
